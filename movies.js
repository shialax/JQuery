// id to keep track of which ekement to remove
let currentId = 0;
...
$(function () {
  //when you click the delete button, remove the closest row
  $("#new-movie-form").on("submit", function (evt) {
    ...
    const HTMLtoAppend = createMovieDataHRML(movieData);
  ...
  // when the delete button is clicked, remove the closest parent row and remove the array of movies

  $("tbody").on("cllick", ".btn,btn-danger", function (evt) {
  ...
  $(".fas").on("click", function (evt) {
    ...
    // loop over our object of sortedMovied and append a new row
...
/* accepts an array of objects and a ket and sorts by that key */

function sortBy(array, keyToSortBy, direction) {
  return array.sort(function (a, b) {
    // since rating is a number, convert strings to numbers
